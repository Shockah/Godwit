...
import pl.shockah.godwit.gesture.ContinuousGestureRecognizer;
...
import pl.shockah.godwit.gesture.Touch;

public class EntityTreeGestureManager extends GestureManager {
	...
	public void update() {
		...
		if (touches.isEmpty()) {
			...
			if (!inProgress) {
				for (GestureRecognizer recognizer : recognizers) {
					if (recognizer.getState() == GestureRecognizer.State.Detecting)
						recognizer.setState(GestureRecognizer.State.Failed);
					else if (recognizer.isInProgress())
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		handle(GestureRecognizer::handleTouchDown, true, touch, point, ((EntityTreeManager)Godwit.getInstance().manager).getRootEntity());
		return true;
	}
	...
	public boolean touchDragged(int screenX, int screenY, int pointer) {
		...
		Vec2 point = new Vec2(screenX, screenY);
		touch.addPoint(point);
		handle(GestureRecognizer::handleTouchDragged, false, touch, point, ((EntityTreeManager)Godwit.getInstance().manager).getRootEntity());
		return true;
	}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		handle(GestureRecognizer::handleTouchUp, false, touch, point, ((EntityTreeManager)Godwit.getInstance().manager).getRootEntity());
		return true;
	}
